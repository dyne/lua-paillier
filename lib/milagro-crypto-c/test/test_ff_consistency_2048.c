/*
    Licensed to the Apache Software Foundation (ASF) under one
    or more contributor license agreements.  See the NOTICE file
    distributed with this work for additional information
    regarding copyright ownership.  The ASF licenses this file
    to you under the Apache License, Version 2.0 (the
    "License"); you may not use this file except in compliance
    with the License.  You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

    Unless required by applicable law or agreed to in writing,
    software distributed under the License is distributed on an
    "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
    KIND, either express or implied.  See the License for the
    specific language governing permissions and limitations
    under the License.
*/
/**
 * @file test_big_consistency.c
 * @author Samuele Andreoli
 * @brief Test for consistency of FF_2048
 *
 */

#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include "arch.h"
#include "amcl.h"
#include "utils.h"
#include "ff_2048.h"

#define FS_2048  MODBYTES_1024_58*FFLEN_2048
#define HFS_2048 MODBYTES_1024_58*HFLEN_2048

#if 2048 == 2048
char *Phex = "";
char *Qhex = "";
#elif 2048 == 3072
char *Phex = "";
char *Qhex = "";
#else
char *Phex = "";
char *Qhex = "";
#endif

int main()
{

    int i;
    char seed[256];
    csprng RNG;

    char oct[FS_2048];
    octet OCT = {0,FS_2048,oct};

    BIG_1024_58 A[HFLEN_2048], B[HFLEN_2048], C[HFLEN_2048], D[HFLEN_2048];
    BIG_1024_58 E[FFLEN_2048], F[FFLEN_2048], G[FFLEN_2048], H[FFLEN_2048], L[FFLEN_2048], P[FFLEN_2048], Q[FFLEN_2048], N[FFLEN_2048];

    /* Fake random source */
    RAND_clean(&RNG);
    for (i=0; i<256; i++) seed[i]=(char)i;
    RAND_seed(&RNG,256,seed);

    /* Testing general equality and equality to zero */
    FF_2048_zero(A, HFLEN_2048);
    FF_2048_zero(B, HFLEN_2048);

    if(FF_2048_comp(A,B,HFLEN_2048) || !FF_2048_iszilch(A, HFLEN_2048))
    {
        printf("ERROR comparing or setting to zero\n");
        exit(EXIT_FAILURE);
    }

    /* Test parity */
    FF_2048_one(A,HFLEN_2048);
    if (!FF_2048_parity(A) || FF_2048_parity(B))
    {
        printf("ERROR checking parity\n");
        exit(EXIT_FAILURE);
    }

    /* Test last bits */
    FF_2048_init(A,0xF8,HFLEN_2048);
    if(FF_2048_lastbits(A,8) != 0xF8 || FF_2048_lastbits(A,12) != 0xF8)
    {
        printf("ERROR checking last bits\n");
        exit(EXIT_FAILURE);
    }

    /* Testing copy*/
    FF_2048_random(F,&RNG,FFLEN_2048);
    FF_2048_copy(G,F,FFLEN_2048);
    if(FF_2048_comp(G,F,FFLEN_2048))
    {
        printf("ERROR testing copy\n");
        exit(EXIT_FAILURE);
    }

    /* Testing addition, subtraction */
    for (i=0; i<10; i++)
    {
        FF_2048_random(F,&RNG,FFLEN_2048);
        FF_2048_random(G,&RNG,FFLEN_2048);
        FF_2048_copy(H,G,FFLEN_2048);

        FF_2048_add(G,G,F,FFLEN_2048);
        FF_2048_sub(G,G,F,FFLEN_2048);
        FF_2048_sub(F,F,F,FFLEN_2048);
        if(FF_2048_comp(G,H,FFLEN_2048) || !FF_2048_iszilch(F,FFLEN_2048))
        {
            printf("ERROR testing addition/subtraction\n");
            exit(EXIT_FAILURE);
        }
    }

    FF_2048_one(A,HFLEN_2048);
    FF_2048_zero(B,HFLEN_2048);

    FF_2048_add(C,A,B,HFLEN_2048);
    FF_2048_add(B,B,B,HFLEN_2048);
    if(FF_2048_comp(A,C,HFLEN_2048) || !FF_2048_iszilch(B,HFLEN_2048))
    {
        printf("ERROR testing addition/subtraction\n");
        exit(EXIT_FAILURE);
    }

    /* Testing increment/decrement */
    for (i=0; i<10; i++)
    {
        FF_2048_random(F,&RNG,FFLEN_2048);
        FF_2048_copy(H,F,FFLEN_2048);

        FF_2048_inc(F,10,FFLEN_2048);
        FF_2048_dec(F,10,FFLEN_2048);
        if(FF_2048_comp(F,H,FFLEN_2048))
        {
            printf("ERROR testing addition/subtraction\n");
            exit(EXIT_FAILURE);
        }
    }

    FF_2048_zero(A,HFLEN_2048);

    FF_2048_inc(A,0,HFLEN_2048);
    if(!FF_2048_iszilch(A,HFLEN_2048))
    {
        printf("ERROR testing addition/subtraction\n");
        exit(EXIT_FAILURE);
    }

    FF_2048_dec(A,0,HFLEN_2048);
    if(!FF_2048_iszilch(A,HFLEN_2048))
    {
        printf("ERROR testing addition/subtraction\n");
        exit(EXIT_FAILURE);
    }

    /* Testing shifts */
    FF_2048_one(A,HFLEN_2048);
    FF_2048_init(B,2,HFLEN_2048);
    FF_2048_one(C,HFLEN_2048);

    FF_2048_shl(A,HFLEN_2048);
    if(FF_2048_comp(A,B,HFLEN_2048))
    {
        printf("ERROR testing shifts\n");
        exit(EXIT_FAILURE);
    }

    FF_2048_shr(A,HFLEN_2048);
    if(FF_2048_comp(A,C,HFLEN_2048))
    {
        printf("ERROR testing shifts\n");
        exit(EXIT_FAILURE);
    }

    /* Testing octet conversions */
    for (i=0; i<10; i++)
    {
        OCT_clear(&OCT);

        FF_2048_random(F,&RNG,FFLEN_2048);
        FF_2048_copy(H,F,FFLEN_2048);

        FF_2048_toOctet(&OCT,F,FFLEN_2048);
        FF_2048_zero(F,FFLEN_2048);
        FF_2048_fromOctet(F,&OCT,FFLEN_2048);

        if(FF_2048_comp(F,H,FFLEN_2048))
        {
            printf("ERROR testing octet conversion\n");
            exit(EXIT_FAILURE);
        }

        OCT_clear(&OCT);

        FF_2048_random(A,&RNG,HFLEN_2048);
        FF_2048_copy(C,A,HFLEN_2048);

        FF_2048_toOctet(&OCT,A,HFLEN_2048);
        FF_2048_zero(A,HFLEN_2048);
        FF_2048_fromOctet(A,&OCT,HFLEN_2048);

        if(FF_2048_comp(A,C,HFLEN_2048))
        {
            printf("ERROR testing octet conversion\n");
            exit(EXIT_FAILURE);
        }
    }

    /* Testing multiplication/square */
    for (i=0; i<10; i++)
    {
        FF_2048_random(A,&RNG,HFLEN_2048);

        FF_2048_mul(F,A,A,HFLEN_2048);
        FF_2048_sqr(G,A,HFLEN_2048);

        // Normalise for comparison
        FF_2048_norm(F,FFLEN_2048);
        FF_2048_norm(G,FFLEN_2048);

        if(FF_2048_comp(F,G,FFLEN_2048))
        {
            printf("ERROR testing multiplication/square");
            exit(EXIT_FAILURE);
        }
    }

    /* Read P and Q primes from hex */
    OCT_fromHex(&OCT, Phex);
    OCT_pad(&OCT,FS_2048);
    FF_2048_fromOctet(P,&OCT,FFLEN_2048);

    OCT_fromHex(&OCT, Qhex);
    OCT_pad(&OCT,FS_2048);
    FF_2048_fromOctet(Q,&OCT,FFLEN_2048);

    /* Testing inverse modulo P */
    FF_2048_one(C,HFLEN_2048);
    for (i=0; i< 10; i++)
    {
        FF_2048_random(A,&RNG, HFLEN_2048);
        FF_2048_mod(A,P,HFLEN_2048);
        FF_2048_copy(B,A,HFLEN_2048);

        FF_2048_invmodp(B,B,P,HFLEN_2048);
        FF_2048_mul(F,A,B,HFLEN_2048);
        FF_2048_mod(F,P,FFLEN_2048);

        if(FF_2048_comp(C,F,HFLEN_2048))
        {
            printf("ERROR testing inverse modulo p");
            exit(EXIT_FAILURE);
        }
    }

    /* Testing CRT */
    FF_2048_mul(N,P,Q,HFLEN_2048);

    for (i=0; i<10; i++)
    {
        FF_2048_random(A,&RNG,HFLEN_2048);
        FF_2048_random(B,&RNG,HFLEN_2048);

        FF_2048_mul(H,A,B,HFLEN_2048);
        FF_2048_mod(H,N,FFLEN_2048);

        FF_2048_copy(C,A,HFLEN_2048);
        FF_2048_copy(D,B,HFLEN_2048);
        FF_2048_mod(A,P,HFLEN_2048);
        FF_2048_mod(B,P,HFLEN_2048);
        FF_2048_mod(C,Q,HFLEN_2048);
        FF_2048_mod(D,Q,HFLEN_2048);

        FF_2048_mul(F,A,B,HFLEN_2048);
        FF_2048_mod(F,P,FFLEN_2048);

        FF_2048_mul(G,C,D,HFLEN_2048);
        FF_2048_mod(G,Q,FFLEN_2048);

        FF_2048_crt(L,F,G,P,Q,HFLEN_2048);

        if(FF_2048_comp(L,H,HFLEN_2048))
        {
            printf("ERROR testing crt");
            exit(EXIT_FAILURE);
        }
    }

    /* Testing multiple power */
    FF_2048_random(A, &RNG, HFLEN_2048);
    FF_2048_random(B, &RNG, HFLEN_2048);
    FF_2048_random(C, &RNG, HFLEN_2048);
    FF_2048_random(D, &RNG, HFLEN_2048);

    FF_2048_random(E, &RNG, HFLEN_2048);
    FF_2048_random(F, &RNG, HFLEN_2048);
    FF_2048_random(G, &RNG, HFLEN_2048);
    FF_2048_random(H, &RNG, HFLEN_2048);

    FF_2048_pow(L, A, E, P, HFLEN_2048);
    FF_2048_pow(N, B, F, P, HFLEN_2048);
    FF_2048_mul(Q, L, N, HFLEN_2048);
    FF_2048_dmod(L, Q, P, HFLEN_2048);

    FF_2048_bpow2(N, A, E, B, F, P, HFLEN_2048, HFLEN_2048);

    if(FF_2048_comp(N, L, HFLEN_2048))
    {
        printf("ERROR testing bpow2");
        exit(EXIT_FAILURE);
    }

    // Test triple exponent
    FF_2048_pow(N, C, G, P, HFLEN_2048);
    FF_2048_mul(Q, L, N, HFLEN_2048);
    FF_2048_dmod(L, Q, P, HFLEN_2048);

    FF_2048_pow3(N, A, E, B, F, C, G, P, HFLEN_2048, HFLEN_2048);

    if(FF_2048_comp(N, L, HFLEN_2048))
    {
        printf("ERROR testing pow3");
        exit(EXIT_FAILURE);
    }

    // Test triple exponent for secret key
    FF_2048_zero(N, HFLEN_2048);
    FF_2048_skpow3(N, A, E, B, F, C, G, P, HFLEN_2048, HFLEN_2048);

    if(FF_2048_comp(N, L, HFLEN_2048))
    {
        printf("ERROR testing pow3");
        exit(EXIT_FAILURE);
    }

    // Test quadruple exponent
    FF_2048_pow(N, D, H, P, HFLEN_2048);
    FF_2048_mul(Q, L, N, HFLEN_2048);
    FF_2048_dmod(L, Q, P, HFLEN_2048);

    FF_2048_pow4(N, A, E, B, F, C, G, D, H, P, HFLEN_2048, HFLEN_2048);

    if(FF_2048_comp(N, L, HFLEN_2048))
    {
        printf("ERROR testing pow4");
        exit(EXIT_FAILURE);
    }

    printf("SUCCESS TEST CONSISTENCY OF FF_2048 PASSED\n");
    exit(EXIT_SUCCESS);
}
